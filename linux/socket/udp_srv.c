...
{  
    //通过参数传入服务器端要绑定的地址信息（IP地址和端口）
    if (argc != 3) {
        printf("em: ./udp_srv 192.168.231.128 9000\n");
	return -1;  
   }
   ...
   //这一步就注明了本次通信是什么通信 AF_INET-IPV4的通信,并且是UDP协议的数据报传输服务
	...
	//参数=0 指的是阻塞接收数据--若缓冲区中没有数据则需要等待，直到有数据
        ...
        scanf("%s",buf);//从标准输入获取数据放入buf中，然后响应给客户端
